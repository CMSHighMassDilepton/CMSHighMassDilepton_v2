...
paths_data = "/depot/cms/users/kaur214/analysis_facility/outputs/test/stage1_output/2022preEE/f1_0/data_*/*/*parquet"
...
#massBinningMuMu = (
#    [j for j in range(60, 120, 5)]
#    + [120]
#)

massBinningMuMu = (
    [j for j in range(30, 200, 5)]
    + [200]
)
...
data_mass =  df_data["dimuon_mass"].compute().values 
...
for i in range(len(dy_mass)):
    ...
    #h_dy.Fill(dy_mass[i], 0.23*wgt_dy[i])

for i in range(len(data_mass)):
    h_data.Fill(data_mass[i], wgt_data[i])


file2 = TFile("new_dy_2022_mass.root","RECREATE")
file2.cd()
h_dy.Write()
h_data.Write()
file2.Close()
